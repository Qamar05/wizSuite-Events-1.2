...
class UpComingEventsViewModel: ViewModel() {
    ...
    val upcomingEventResponse = MutableLiveData<EventResponse>()
    ...
    fun refreshUpcomingEvent(token: String) {
        //if (isUserNameValid(email)) {
        requestUpcomingEvents(EventRequest(token))
        /*} else {
            sendOTPError.value = "Please enter valid Email"
        }*/
    }

    private fun requestUpcomingEvents(request: EventRequest) {
        loading.value = true
        job = CoroutineScope(Dispatchers.IO + exceptionHandler).launch {
            supervisorScope {
                try {
                    val response = apiService.requestUpcomingEvents(request)
                    withContext(Dispatchers.Main) {
                        ...
                        if (response.code() == 200) {
                            ...
                            upcomingEventResponse.value = response.body()
