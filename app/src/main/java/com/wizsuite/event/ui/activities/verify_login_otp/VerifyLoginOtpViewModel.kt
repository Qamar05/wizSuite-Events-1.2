...
class VerifyLoginOtpViewModel : ViewModel() {
    private val apiService = ApiService.getNetworkService()
    private var job: Job? = null


    val smsLoginResponse = MutableLiveData<LoginResponse>()
    ...
    fun refreshSmsLogin(phone: String, otp : String) {

        requestSmsLogin(VerifyLoginOTPRequest(phone, otp))

    }

    private fun requestSmsLogin(request: VerifyLoginOTPRequest) {
        loading.value = true
        job = CoroutineScope(Dispatchers.IO + exceptionHandler).launch {
            supervisorScope {
                try {
                    val response = apiService.requestVerifySmsOtp(request)
                    withContext(Dispatchers.Main) {
                        ...
                        if (response.code() == 200) {
                            ...
                            smsLoginResponse.value = response.body()
