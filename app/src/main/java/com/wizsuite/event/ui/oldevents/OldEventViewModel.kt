...
class OldEventViewModel : ViewModel() {

    private val apiService = ApiService.getNetworkService()
    private var job: Job? = null


    val pastEventResponse = MutableLiveData<EventResponse>()
    ...
    fun refreshPastEvent(token: String) {
        //if (isUserNameValid(email)) {
        requestPastEvents(EventRequest(token))
        /*} else {
            sendOTPError.value = "Please enter valid Email"
        }*/
    }

    private fun requestPastEvents(request: EventRequest) {
        loading.value = true
        job = CoroutineScope(Dispatchers.IO + exceptionHandler).launch {
            supervisorScope {
                try {
                    val response = apiService.requestPastEvents(request)
                    withContext(Dispatchers.Main) {
                        ...
                        if (response.code() == 200) {
                            ...
                            pastEventResponse.value = response.body()
